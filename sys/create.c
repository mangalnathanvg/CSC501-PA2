...
/* emulate 386 "pushal" instruction */
	...
	*pushsp = pptr->pesp = (unsigned long)saddr;
	...
	// Create page directory entry for process other than NULLPROC
...
void create_page_directory(int pid)
{
	...
	// Obtain a free frame to allocate to page directory
	int frame_number;
	get_frm(&frame_number);

	// kprintf("\nPage Directory at Frame %d for Process %d \n", frame_number, currpid);
	...
	// Set PDBR for process created
	proctab[pid].pdbr = (FRAME0 + frame_number) * NBPG;

	// Set base address of page directory of process
	page_directory_entry = proctab[pid].pdbr;

	int i;
	// Initialize 4 entries (referencing page tables) for page directory. The rest of the entries are not present but writeable.
	for(i=0; i < NFRAMES; i++)
	{
